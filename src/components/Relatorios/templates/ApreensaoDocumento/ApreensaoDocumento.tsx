import {dateFormat} from '../../../../utils/apex-formatters'

interface IProps {
    data: any
}

export const ApreensaoDocumento: React.FC<IProps> = (props) => {
    const {data} = props;

    const hiddenCss = {
        fontWeight: 600,
        opacity: 0,
        position: "fixed" as "fixed",
        left: "-50%"
    }

    console.log("ApreensaoDocumento: ", data)   
    return (
        <table style={hiddenCss} width="100%" cellPadding={0} cellSpacing={0}>
            <tbody>
            <tr>
                <td width="50%"/>
                <td align="center">
                    <table
                        id="JR_PAGE_ANCHOR_APREENSAO_DOC_0_1"
                        role="none"
                        className="jrPage"
                        data-jr-height={842}
                        cellPadding={0}
                        cellSpacing={0}

                        style={{
                            emptyCells: "show",
                            width: 595,
                            borderCollapse: "collapse",
                            backgroundColor: "white"
                        }}
                    >
                        <style
                            type="text/css"
                            dangerouslySetInnerHTML={{
                                __html:
                                    "\n                    #JR_PAGE_ANCHOR_APREENSAO_DOC_0_1 th {\n                        font-weight: normal;\n                    }\n\n                    #JR_PAGE_ANCHOR_APREENSAO_DOC_0_1 ul {\n                        list-style-type: disc;\n                        padding-inline-start: 40px;\n                        margin: 0px;\n                    }\n\n                    #JR_PAGE_ANCHOR_APREENSAO_DOC_0_1 ol {\n                        list-style-type: decimal;\n                        padding-inline-start: 40px;\n                        margin: 0px;\n                    }\n                "
                            }}
                        />
                        <tbody>
                        <tr role="none" style={{height: 0}}>
                            <td style={{width: 20}}></td>
                            <td style={{width: 60}}></td>
                            <td style={{width: 10}}></td>
                            <td style={{width: 40}}></td>
                            <td style={{width: 20}}></td>
                            <td style={{width: 10}}></td>
                            <td style={{width: 30}}></td>
                            <td style={{width: 5}}></td>
                            <td style={{width: 1}}></td>
                            <td style={{width: 64}}></td>
                            <td style={{width: 30}}></td>
                            <td style={{width: 10}}></td>
                            <td style={{width: 48}}></td>
                            <td style={{width: 38}}></td>
                            <td style={{width: 1}}></td>
                            <td style={{width: 90}}></td>
                            <td style={{width: 3}}></td>
                            <td style={{width: 54}}></td>
                            <td style={{width: 40}}></td>
                            <td style={{width: 21}}></td>
                        </tr>
                        <tr style={{height: 20}}>
                            <td colSpan={20}/>
                        </tr>
                        <tr style={{height: 83}}>
                            <td/>
                            <td colSpan={18}>
                                <div
                                    style={{
                                        width: "100%",
                                        height: "100%",
                                        position: "relative"
                                    }}
                                >
                                    <div
                                        style={{
                                            position: "absolute",
                                            overflow: "hidden",
                                            width: "100%",
                                            height: "100%"
                                        }}
                                    >
                                        <table
                                            cellPadding={0}
                                            cellSpacing={0}

                                            style={{
                                                emptyCells: "show",
                                                width: "100%",
                                                borderCollapse: "collapse"
                                            }}
                                        >
                                            <tbody>
                                            <tr role="none" style={{height: 0}}>
                                                <td style={{width: 554}}></td>
                                            </tr>
                                            <tr style={{height: 80}}>
                                                <td/>
                                            </tr>
                                            <tr style={{height: 1}}>
                                                <td
                                                    style={{
                                                        pointerEvents: "auto",
                                                        backgroundColor: "#FFFFFF",
                                                        borderTop: "1px solid #000000"
                                                    }}
                                                />
                                            </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                    <div
                                        style={{
                                            position: "relative",
                                            width: "100%",
                                            height: "100%",
                                            pointerEvents: "none"
                                        }}
                                    >
                                        <table
                                            cellPadding={0}
                                            cellSpacing={0}

                                            style={{
                                                emptyCells: "show",
                                                width: "100%",
                                                borderCollapse: "collapse"
                                            }}
                                        >
                                            <tbody>
                                            <tr role="none" style={{height: 0}}>
                                                <td style={{width: 280}}></td>
                                                <td style={{width: 100}}></td>
                                                <td style={{width: 174}}></td>
                                            </tr>
                                            <tr style={{height: 3}}>
                                                <td/>
                                                <td rowSpan={7}>
                                                    <div
                                                        style={{
                                                            width: "100%",
                                                            height: "100%",
                                                            position: "relative"
                                                        }}
                                                    >
                                                        <div
                                                            style={{
                                                                position: "absolute",
                                                                overflow: "hidden",
                                                                width: "100%",
                                                                height: "100%"
                                                            }}
                                                        >
                                                            <table
                                                                cellPadding={0}
                                                                cellSpacing={0}

                                                                style={{
                                                                    emptyCells: "show",
                                                                    width: "100%",
                                                                    borderCollapse: "collapse"
                                                                }}
                                                            >
                                                                <tbody>
                                                                <tr
                                                                    role="none"

                                                                    style={{height: 0}}
                                                                >
                                                                    <td style={{width: 1}}></td>
                                                                    <td style={{width: 99}}></td>
                                                                </tr>
                                                                <tr style={{height: 80}}>
                                                                    <td
                                                                        style={{
                                                                            pointerEvents: "auto",
                                                                            backgroundColor: "#FFFFFF",
                                                                            borderLeft: "1px solid #000000"
                                                                        }}
                                                                    />
                                                                    <td/>
                                                                </tr>
                                                                </tbody>
                                                            </table>
                                                        </div>
                                                        <div
                                                            style={{
                                                                position: "relative",
                                                                width: "100%",
                                                                height: "100%",
                                                                pointerEvents: "none"
                                                            }}
                                                        >
                                                            <table
                                                                cellPadding={0}
                                                                cellSpacing={0}

                                                                style={{
                                                                    emptyCells: "show",
                                                                    width: "100%",
                                                                    borderCollapse: "collapse"
                                                                }}
                                                            >
                                                                <tbody>
                                                                <tr
                                                                    role="none"

                                                                    style={{height: 0}}
                                                                >
                                                                    <td style={{width: 100}}></td>
                                                                </tr>
                                                                <tr style={{height: 14}}>
                                                                    <td/>
                                                                </tr>
                                                                <tr style={{height: 19}}>
                                                                    <td
                                                                        style={{
                                                                            pointerEvents: "auto",
                                                                            textIndent: 0,
                                                                            verticalAlign: "middle",
                                                                            textAlign: "right"
                                                                        }}
                                                                    >
                                                                        <div style={{paddingRight: 5}}>
                                            <span
                                                style={{
                                                    fontFamily:
                                                        '"DejaVu Sans", Arial, Helvetica, sans-serif',
                                                    color: "#000000",
                                                    fontSize: 12,
                                                    lineHeight: "normal",
                                                    fontWeight: "bold"
                                                }}
                                            >
                                              Unidade:
                                            </span>
                                                                        </div>
                                                                    </td>
                                                                </tr>
                                                                <tr style={{height: 7}}>
                                                                    <td/>
                                                                </tr>
                                                                <tr style={{height: 19}}>
                                                                    <td
                                                                        style={{
                                                                            pointerEvents: "auto",
                                                                            textIndent: 0,
                                                                            verticalAlign: "middle",
                                                                            textAlign: "right"
                                                                        }}
                                                                    >
                                                                        <div style={{paddingRight: 5}}>
                                            <span
                                                style={{
                                                    fontFamily:
                                                        '"DejaVu Sans", Arial, Helvetica, sans-serif',
                                                    color: "#000000",
                                                    fontSize: 12,
                                                    lineHeight: "normal",
                                                    fontWeight: "bold"
                                                }}
                                            >
                                              NP:
                                            </span>
                                                                        </div>
                                                                    </td>
                                                                </tr>

                                                                <tr style={{height: 1}}>
                                                                    <td/>
                                                                </tr>
                                                                <tr style={{height: 19}}>
                                                                    <td
                                                                        style={{
                                                                            pointerEvents: "auto",
                                                                            textIndent: 0,
                                                                            verticalAlign: "middle",
                                                                            textAlign: "right"
                                                                        }}
                                                                    >
                                                                        <div style={{paddingRight: 5}}>
                                            <span
                                                style={{
                                                    fontFamily:
                                                        '"DejaVu Sans", Arial, Helvetica, sans-serif',
                                                    color: "#000000",
                                                    fontSize: 12,
                                                    lineHeight: "normal",
                                                    fontWeight: "bold"
                                                }}
                                            >
                                              AUTO:
                                            </span>
                                                                        </div>
                                                                    </td>
                                                                </tr>
                                                                <tr style={{height: 1}}>
                                                                    <td/>
                                                                </tr>
                                                                </tbody>
                                                            </table>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td/>
                                            </tr>
                                            <tr style={{height: 36}}>
                                                <td rowSpan={7}>
                                                    <div
                                                        style={{
                                                            width: "100%",
                                                            height: "100%",
                                                            position: "relative"
                                                        }}
                                                    >
                                                        <div
                                                            style={{
                                                                position: "absolute",
                                                                overflow: "hidden",
                                                                width: "100%",
                                                                height: "100%"
                                                            }}
                                                        >
                                                            <table
                                                                cellPadding={0}
                                                                cellSpacing={0}

                                                                style={{
                                                                    emptyCells: "show",
                                                                    width: "100%",
                                                                    borderCollapse: "collapse"
                                                                }}
                                                            >
                                                                <tbody>
                                                                <tr
                                                                    role="none"

                                                                    style={{height: 0}}
                                                                >
                                                                    <td style={{width: 280}}></td>
                                                                </tr>
                                                                <tr style={{height: 43}}>
                                                                    <td/>
                                                                </tr>
                                                                <tr style={{height: 20}}>
                                                                    <td
                                                                        style={{
                                                                            pointerEvents: "auto",
                                                                            textIndent: 0,
                                                                            verticalAlign: "middle",
                                                                            textAlign: "center"
                                                                        }}
                                                                    >
                                          <span
                                              style={{
                                                  fontFamily:
                                                      '"DejaVu Sans", Arial, Helvetica, sans-serif',
                                                  color: "#000000",
                                                  fontSize: 14,
                                                  lineHeight: "1.1640625",
                                                  fontWeight: "bold"
                                              }}
                                          >
                                            Ministério da Administração
                                          </span>
                                                                    </td>
                                                                </tr>
                                                                </tbody>
                                                            </table>
                                                        </div>
                                                        <div
                                                            style={{
                                                                position: "relative",
                                                                width: "100%",
                                                                height: "100%",
                                                                pointerEvents: "none"
                                                            }}
                                                        >
                                                            <table
                                                                cellPadding={0}
                                                                cellSpacing={0}

                                                                style={{
                                                                    emptyCells: "show",
                                                                    width: "100%",
                                                                    borderCollapse: "collapse"
                                                                }}
                                                            >
                                                                <tbody>
                                                                <tr
                                                                    role="none"

                                                                    style={{height: 0}}
                                                                >
                                                                    <td style={{width: 110}}></td>
                                                                    <td style={{width: 50}}></td>
                                                                    <td style={{width: 120}}></td>
                                                                </tr>
                                                                <tr style={{height: 45}}>
                                                                    <td/>
                                                                    <td
                                                                        style={{
                                                                            textAlign: "center",
                                                                            verticalAlign: "middle",
                                                                            pointerEvents: "auto"
                                                                        }}
                                                                    ><img style={{
                                                                        position: "absolute",
                                                                        top: "-14px",
                                                                        width: "auto",
                                                                        height: 59
                                                                    }}
                                                                          src={"assets/icon/relatorios/BRASAO.png"}/>
                                                                    </td>
                                                                    <td/>
                                                                </tr>
                                                                <tr style={{height: 15}}>
                                                                    <td colSpan={3}/>
                                                                </tr>
                                                                <tr style={{height: 20}}>
                                                                    <td
                                                                        colSpan={3}
                                                                        style={{
                                                                            pointerEvents: "auto",
                                                                            textIndent: 0,
                                                                            verticalAlign: "middle",
                                                                            textAlign: "center"
                                                                        }}
                                                                    >
                                          <span
                                              style={{
                                                  fontFamily:
                                                      "Arial Rounded MT Bold",
                                                  color: "#000000",
                                                  fontSize: 12,
                                                  lineHeight: "1.1640625"
                                              }}
                                          >
                                            GUARDA NACIONAL REPUBLICANA
                                          </span>
                                                                    </td>
                                                                </tr>
                                                                </tbody>
                                                            </table>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td
                                                    style={{
                                                        pointerEvents: "auto",
                                                        textIndent: 0,
                                                        textAlign: "left",
                                                        fontSize: 12,
                                                        height: 36,
                                                        overflow: "hidden"
                                                    }}
                                                >
                                                    DIVISÃO DE TRÂNSITO E SEGURANÇA RODOVIÁRIA
                                                </td>
                                            </tr>
                                            <tr style={{height: 1}}>
                                                <td/>
                                            </tr>
                                            <tr style={{height: 19}}>
                                                <td
                                                    style={{
                                                        pointerEvents: "auto",
                                                        textIndent: 0,
                                                        textAlign: "left"
                                                    }}
                                                />
                                            </tr>
                                            <tr style={{height: 1}}>
                                                <td/>
                                            </tr>
                                            <tr style={{height: 19}}>
                                                <td
                                                    style={{
                                                        pointerEvents: "auto",
                                                        textIndent: 0,
                                                        textAlign: "left"
                                                    }}
                                                />
                                            </tr>
                                            <tr style={{height: 1}}>
                                                <td/>
                                            </tr>
                                            <tr style={{height: 3}}>
                                                <td colSpan={2}/>
                                            </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                            </td>
                            <td/>
                        </tr>
                        <tr style={{height: 8}}>
                            <td colSpan={20}/>
                        </tr>
                        <tr style={{height: 19}}>
                            <td/>
                            <td colSpan={18} style={{textIndent: 0, textAlign: "center"}}>
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: 16,
                          lineHeight: "1.1640625"
                      }}
                  >
                    Auto de Apreensão de Documentos
                  </span>
                            </td>
                            <td/>
                        </tr>
                        <tr style={{height: 10}}>
                            <td colSpan={20}/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td
                                colSpan={5}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: 12,
                          lineHeight: "1.1640625"
                      }}
                  >
                    Dados da Ocorrência
                  </span>
                            </td>
                            <td colSpan={14}/>
                        </tr>
                        <tr style={{height: 21}}>
                            <td/>
                            <td colSpan={18}>
                                <div
                                    style={{
                                        width: "100%",
                                        height: "100%",
                                        position: "relative"
                                    }}
                                >
                                    <div
                                        style={{
                                            position: "absolute",
                                            overflow: "hidden",
                                            width: "100%",
                                            height: "100%"
                                        }}
                                    >
                                        <table
                                            cellPadding={0}
                                            cellSpacing={0}

                                            style={{
                                                emptyCells: "show",
                                                width: "100%",
                                                borderCollapse: "collapse"
                                            }}
                                        >
                                            <tbody>
                                            <tr role="none" style={{height: 0}}>
                                                <td style={{width: 554}}></td>
                                            </tr>
                                            <tr style={{height: 1}}>
                                                <td
                                                    style={{
                                                        pointerEvents: "auto",
                                                        backgroundColor: "#FFFFFF",
                                                        borderTop: "1px solid #000000"
                                                    }}
                                                />
                                            </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                    <div
                                        style={{
                                            position: "relative",
                                            width: "100%",
                                            height: "100%",
                                            pointerEvents: "none"
                                        }}
                                    >
                                        <table
                                            cellPadding={0}
                                            cellSpacing={0}

                                            style={{
                                                emptyCells: "show",
                                                width: "100%",
                                                borderCollapse: "collapse"
                                            }}
                                        >
                                            <tbody>
                                            <tr role="none" style={{height: 0}}>
                                                <td style={{width: 65}}></td>
                                                <td style={{width: 215}}></td>
                                                <td style={{width: 59}}></td>
                                                <td style={{width: 215}}></td>
                                            </tr>
                                            <tr style={{height: 1}}>
                                                <td
                                                    rowSpan={2}
                                                    style={{
                                                        pointerEvents: "auto",
                                                        textIndent: 0,
                                                        verticalAlign: "middle",
                                                        textAlign: "left"
                                                    }}
                                                >
                              <span
                                  style={{
                                      fontFamily: "Arial Rounded MT Bold",
                                      color: "#000000",
                                      fontSize: 12,
                                      lineHeight: "1.1640625"
                                  }}
                              >
                                Data/Hora:
                              </span>
                                                </td>
                                                <td
                                                    rowSpan={2}
                                                    style={{
                                                        pointerEvents: "auto",
                                                        textIndent: 0,
                                                        verticalAlign: "middle",
                                                        textAlign: "left"
                                                    }}
                                                >{dateFormat(data?.infracao?.unidadeData?.dataHoraInfraccao, 'YYYY-MM-DD[/]HH[h]mm')}</td>
                                                <td
                                                    rowSpan={2}
                                                    style={{
                                                        pointerEvents: "auto",
                                                        textIndent: 0,
                                                        verticalAlign: "middle",
                                                        textAlign: "left"
                                                    }}
                                                >
                              <span
                                  style={{
                                      fontFamily: "Arial Rounded MT Bold",
                                      color: "#000000",
                                      fontSize: 12,
                                      lineHeight: "1.1640625"
                                  }}
                              >
                                Autuante:
                              </span>
                                                </td>
                                                <td/>
                                            </tr>
                                            <tr style={{height: 20}}>
                                                <td
                                                    style={{
                                                        pointerEvents: "auto",
                                                        textIndent: 0,
                                                        verticalAlign: "middle",
                                                        textAlign: "left"
                                                    }}
                                                >{data?.infracao?.infracaoData?.autuante}</td>
                                            </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                            </td>
                            <td/>
                        </tr>
                        <tr style={{height: 59}}>
                            <td/>
                            <td colSpan={11}>
                                <div
                                    style={{
                                        width: "100%",
                                        height: "100%",
                                        position: "relative"
                                    }}
                                >
                                    <div
                                        style={{
                                            position: "absolute",
                                            overflow: "hidden",
                                            width: "100%",
                                            height: "100%"
                                        }}
                                    >
                                        <table
                                            cellPadding={0}
                                            cellSpacing={0}

                                            style={{
                                                emptyCells: "show",
                                                width: "100%",
                                                borderCollapse: "collapse"
                                            }}
                                        >
                                            <tbody>
                                            <tr role="none" style={{height: 0}}>
                                                <td style={{width: 40}}></td>
                                                <td style={{width: 240}}></td>
                                            </tr>
                                            <tr style={{height: 20}}>
                                                <td
                                                    style={{
                                                        pointerEvents: "auto",
                                                        textIndent: 0,
                                                        verticalAlign: "middle",
                                                        textAlign: "left"
                                                    }}
                                                >
                              <span
                                  style={{
                                      fontFamily: "Arial Rounded MT Bold",
                                      color: "#000000",
                                      fontSize: 12,
                                      lineHeight: "1.1640625"
                                  }}
                              >
                                Local:
                              </span>
                                                </td>
                                                <td>{data?.infracao?.localInfracaoData?.tipo?.descricao}, {data?.infracao?.localInfracaoData?.arruamento}, {data?.infracao?.localInfracaoData?.numeroPolicia}, {data?.infracao?.localInfracaoData?.localidade}</td>
                                            </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                    <div
                                        style={{
                                            position: "absolute",
                                            overflow: "hidden",
                                            width: "100%",
                                            height: "100%",
                                            pointerEvents: "none"
                                        }}
                                    >
                                        <table
                                            cellPadding={0}
                                            cellSpacing={0}

                                            style={{
                                                emptyCells: "show",
                                                width: "100%",
                                                borderCollapse: "collapse"
                                            }}
                                        >
                                            <tbody>
                                            <tr role="none" style={{height: 0}}>
                                                <td style={{width: 280}}></td>
                                            </tr>
                                            <tr style={{height: 40}}>
                                                <td
                                                    style={{
                                                        pointerEvents: "auto",
                                                        textIndent: 45,
                                                        textAlign: "left"
                                                    }}
                                                />
                                            </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                    <div
                                        style={{
                                            position: "relative",
                                            width: "100%",
                                            height: "100%",
                                            pointerEvents: "none"
                                        }}
                                    >
                                        <table
                                            cellPadding={0}
                                            cellSpacing={0}

                                            style={{
                                                emptyCells: "show",
                                                width: "100%",
                                                borderCollapse: "collapse"
                                            }}
                                        >
                                            <tbody>
                                            <tr role="none" style={{height: 0}}>
                                                <td style={{width: 140}}></td>
                                                <td style={{width: 140}}></td>
                                            </tr>
                                            <tr style={{height: 39}}>
                                                <td colSpan={2}/>
                                            </tr>
                                            <tr style={{height: 20}}>
                                                <td
                                                    style={{
                                                        pointerEvents: "auto",
                                                        textIndent: 0,
                                                        verticalAlign: "middle",
                                                        textAlign: "left"
                                                    }}
                                                >
                              <span
                                  style={{
                                      fontFamily: "Arial Rounded MT Bold",
                                      color: "#000000",
                                      fontSize: 12,
                                      lineHeight: "1.1640625"
                                  }}
                              >
                                Dados da Apreensão
                              </span>
                                                </td>
                                                <td/>
                                            </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                            </td>
                            <td colSpan={8}/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td colSpan={18}>
                                <div
                                    style={{
                                        width: "100%",
                                        height: "100%",
                                        position: "relative"
                                    }}
                                >
                                    <div
                                        style={{
                                            position: "absolute",
                                            overflow: "hidden",
                                            width: "100%",
                                            height: "100%"
                                        }}
                                    >
                                        <table
                                            cellPadding={0}
                                            cellSpacing={0}

                                            style={{
                                                emptyCells: "show",
                                                width: "100%",
                                                borderCollapse: "collapse"
                                            }}
                                        >
                                            <tbody>
                                            <tr role="none" style={{height: 0}}>
                                                <td style={{width: 554}}></td>
                                            </tr>
                                            <tr style={{height: 1}}>
                                                <td
                                                    style={{
                                                        pointerEvents: "auto",
                                                        backgroundColor: "#FFFFFF",
                                                        borderTop: "1px solid #000000"
                                                    }}
                                                />
                                            </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                    <div
                                        style={{
                                            position: "relative",
                                            width: "100%",
                                            height: "100%",
                                            pointerEvents: "none"
                                        }}
                                    >
                                        <table
                                            cellPadding={0}
                                            cellSpacing={0}

                                            style={{
                                                emptyCells: "show",
                                                width: "100%",
                                                borderCollapse: "collapse"
                                            }}
                                        >
                                            <tbody>
                                            <tr role="none" style={{height: 0}}>
                                                <td style={{width: 60}}></td>
                                                <td style={{width: 494}}></td>
                                            </tr>
                                            <tr style={{height: 20}}>
                                                <td
                                                    style={{
                                                        pointerEvents: "auto",
                                                        textIndent: 0,
                                                        verticalAlign: "middle",
                                                        textAlign: "left"
                                                    }}
                                                >
                              <span
                                  style={{
                                      fontFamily: "Arial Rounded MT Bold",
                                      color: "#000000",
                                      fontSize: 12,
                                      lineHeight: "1.1640625"
                                  }}
                              >
                                Motivos:
                              </span>
                                                </td>
                                                <td/>
                                            </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                            </td>
                            <td/>
                        </tr>
                        <tr style={{height: 19}}>
                            <td/>
                            <td
                                colSpan={18}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >{data?.accoesCom?.motivosApreensao}
                            </td>
                            <td/>
                        </tr>
                        <tr style={{height: 1}}>
                            <td colSpan={20}/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td
                                colSpan={5}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: 12,
                          lineHeight: "1.1640625"
                      }}
                  >
                    Normas Infringidas:
                  </span>
                            </td>
                            <td colSpan={14}/>
                        </tr>
                        <tr style={{height: 1}}>
                            <td colSpan={20}/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td
                                colSpan={18}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            />
                            <td/>
                        </tr>
                        <tr style={{height: 9}}>
                            <td colSpan={20}/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td
                                colSpan={2}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: 12,
                          lineHeight: "1.1640625"
                      }}
                  >
                    Veículo
                  </span>
                            </td>
                            <td colSpan={17}/>
                        </tr>
                        <tr style={{height: 1}}>
                            <td/>
                            <td colSpan={18} style={{borderTop: "1px solid #000000"}}/>
                            <td/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: 12,
                          lineHeight: "1.1640625"
                      }}
                  >
                    Matrícula:
                  </span>
                            </td>
                            <td
                                colSpan={6}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >{data?.intervenientes?.veiculo?.matricula}</td>
                            <td/>
                            <td
                                colSpan={2}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: 12,
                          lineHeight: "1.1640625"
                      }}
                  >
                    País de origem:
                  </span>
                            </td>
                            <td
                                colSpan={3}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >{data?.intervenientes?.veiculo?.pais?.descricao}</td>
                            <td/>
                            <td
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: 12,
                          lineHeight: "1.1640625"
                      }}
                  >
                    Marca/Modelo:
                  </span>
                            </td>
                            <td
                                colSpan={3}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            />
                            <td/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td
                                colSpan={3}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: 12,
                          lineHeight: "1.1640625"
                      }}
                  >
                    Categoria/Classe:
                  </span>
                            </td>
                            <td
                                colSpan={7}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            />
                            <td
                                colSpan={3}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: 12,
                          lineHeight: "1.1640625"
                      }}
                  >
                    Tipo/Subclasse:
                  </span>
                            </td>
                            <td/>
                            <td
                                colSpan={4}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            />
                            <td/>
                        </tr>
                        <tr style={{height: 8}}>
                            <td colSpan={20}/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td
                                colSpan={2}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: 12,
                          lineHeight: "1.1640625"
                      }}
                  >
                    Arguido
                  </span>
                            </td>
                            <td colSpan={17}/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td colSpan={18}>
                                <div
                                    style={{
                                        width: "100%",
                                        height: "100%",
                                        position: "relative"
                                    }}
                                >
                                    <div
                                        style={{
                                            position: "absolute",
                                            overflow: "hidden",
                                            width: "100%",
                                            height: "100%"
                                        }}
                                    >
                                        <table
                                            cellPadding={0}
                                            cellSpacing={0}

                                            style={{
                                                emptyCells: "show",
                                                width: "100%",
                                                borderCollapse: "collapse"
                                            }}
                                        >
                                            <tbody>
                                            <tr role="none" style={{height: 0}}>
                                                <td style={{width: 554}}></td>
                                            </tr>
                                            <tr style={{height: 1}}>
                                                <td
                                                    style={{
                                                        pointerEvents: "auto",
                                                        backgroundColor: "#FFFFFF",
                                                        borderTop: "1px solid #000000"
                                                    }}
                                                />
                                            </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                    <div
                                        style={{
                                            position: "relative",
                                            width: "100%",
                                            height: "100%",
                                            pointerEvents: "none"
                                        }}
                                    >
                                        <table
                                            cellPadding={0}
                                            cellSpacing={0}

                                            style={{
                                                emptyCells: "show",
                                                width: "100%",
                                                borderCollapse: "collapse"
                                            }}
                                        >
                                            <tbody>
                                            <tr role="none" style={{height: 0}}>
                                                <td style={{width: 40}}></td>
                                                <td style={{width: 514}}></td>
                                            </tr>
                                            <tr style={{height: 1}}>
                                                <td
                                                    rowSpan={2}
                                                    style={{
                                                        pointerEvents: "auto",
                                                        textIndent: 0,
                                                        verticalAlign: "middle",
                                                        textAlign: "left"
                                                    }}
                                                >
                              <span
                                  style={{
                                      fontFamily: "Arial Rounded MT Bold",
                                      color: "#000000",
                                      fontSize: 12,
                                      lineHeight: "1.1640625"
                                  }}
                              >
                                Nome:
                              </span>
                                                </td>
                                                <td/>
                                            </tr>
                                            <tr style={{height: 19}}>
                                                <td
                                                    style={{
                                                        pointerEvents: "auto",
                                                        textIndent: 0,
                                                        verticalAlign: "middle",
                                                        textAlign: "left"
                                                    }}
                                                />
                                            </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                            </td>
                            <td/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: 12,
                          lineHeight: "1.1640625"
                      }}
                  >
                    Morada:
                  </span>
                            </td>
                            <td
                                colSpan={17}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            />
                            <td/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td
                                colSpan={6}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: 12,
                          lineHeight: "1.1640625"
                      }}
                  >
                    Documento de
                  </span>
                            </td>
                            <td
                                colSpan={12}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            />
                            <td/>
                        </tr>
                        <tr style={{height: 21}}>
                            <td colSpan={20}/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td
                                colSpan={2}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: 12,
                          lineHeight: "1.1640625"
                      }}
                  >
                    Condutor
                  </span>
                            </td>
                            <td colSpan={17}/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td colSpan={18}>
                                <div
                                    style={{
                                        width: "100%",
                                        height: "100%",
                                        position: "relative"
                                    }}
                                >
                                    <div
                                        style={{
                                            position: "absolute",
                                            overflow: "hidden",
                                            width: "100%",
                                            height: "100%"
                                        }}
                                    >
                                        <table
                                            cellPadding={0}
                                            cellSpacing={0}

                                            style={{
                                                emptyCells: "show",
                                                width: "100%",
                                                borderCollapse: "collapse"
                                            }}
                                        >
                                            <tbody>
                                            <tr role="none" style={{height: 0}}>
                                                <td style={{width: 554}}></td>
                                            </tr>
                                            <tr style={{height: 1}}>
                                                <td
                                                    style={{
                                                        pointerEvents: "auto",
                                                        backgroundColor: "#FFFFFF",
                                                        borderTop: "1px solid #000000"
                                                    }}
                                                />
                                            </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                    <div
                                        style={{
                                            position: "relative",
                                            width: "100%",
                                            height: "100%",
                                            pointerEvents: "none"
                                        }}
                                    >
                                        <table
                                            cellPadding={0}
                                            cellSpacing={0}

                                            style={{
                                                emptyCells: "show",
                                                width: "100%",
                                                borderCollapse: "collapse"
                                            }}
                                        >
                                            <tbody>
                                            <tr role="none" style={{height: 0}}>
                                                <td style={{width: 40}}></td>
                                                <td style={{width: 514}}></td>
                                            </tr>
                                            <tr style={{height: 1}}>
                                                <td
                                                    rowSpan={2}
                                                    style={{
                                                        pointerEvents: "auto",
                                                        textIndent: 0,
                                                        verticalAlign: "middle",
                                                        textAlign: "left"
                                                    }}
                                                >
                              <span
                                  style={{
                                      fontFamily: "Arial Rounded MT Bold",
                                      color: "#000000",
                                      fontSize: 12,
                                      lineHeight: "1.1640625"
                                  }}
                              >
                                Nome:
                              </span>
                                                </td>
                                                <td/>
                                            </tr>
                                            <tr style={{height: 19}}>
                                                <td
                                                    style={{
                                                        pointerEvents: "auto",
                                                        textIndent: 0,
                                                        verticalAlign: "middle",
                                                        textAlign: "left"
                                                    }}
                                                />
                                            </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                            </td>
                            <td/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: 12,
                          lineHeight: "1.1640625"
                      }}
                  >
                    Morada:
                  </span>
                            </td>
                            <td
                                colSpan={17}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            />
                            <td/>
                        </tr>
                        <tr style={{height: 10}}>
                            <td colSpan={20}/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td
                                colSpan={8}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: 12,
                          lineHeight: "1.1640625"
                      }}
                  >
                    Documentos Apreendidos
                  </span>
                            </td>
                            <td colSpan={11}/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td colSpan={18}>
                                <div
                                    style={{
                                        width: "100%",
                                        height: "100%",
                                        position: "relative"
                                    }}
                                >
                                    <div
                                        style={{
                                            position: "absolute",
                                            overflow: "hidden",
                                            width: "100%",
                                            height: "100%"
                                        }}
                                    >
                                        <table
                                            cellPadding={0}
                                            cellSpacing={0}

                                            style={{
                                                emptyCells: "show",
                                                width: "100%",
                                                borderCollapse: "collapse"
                                            }}
                                        >
                                            <tbody>
                                            <tr role="none" style={{height: 0}}>
                                                <td style={{width: 80}}></td>
                                                <td style={{width: 474}}></td>
                                            </tr>
                                            <tr style={{height: 20}}>
                                                <td
                                                    style={{
                                                        pointerEvents: "auto",
                                                        textIndent: 0,
                                                        verticalAlign: "middle",
                                                        textAlign: "left"
                                                    }}
                                                >
                              <span
                                  style={{
                                      fontFamily: "Arial Rounded MT Bold",
                                      color: "#000000",
                                      fontSize: 12,
                                      lineHeight: "1.1640625"
                                  }}
                              >
                                Documento:
                              </span>
                                                </td>
                                                <td
                                                    style={{
                                                        pointerEvents: "auto",
                                                        textIndent: 0,
                                                        verticalAlign: "middle",
                                                        textAlign: "left"
                                                    }}
                                                />
                                            </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                    <div
                                        style={{
                                            position: "relative",
                                            width: "100%",
                                            height: "100%",
                                            pointerEvents: "none"
                                        }}
                                    >
                                        <table
                                            cellPadding={0}
                                            cellSpacing={0}

                                            style={{
                                                emptyCells: "show",
                                                width: "100%",
                                                borderCollapse: "collapse"
                                            }}
                                        >
                                            <tbody>
                                            <tr role="none" style={{height: 0}}>
                                                <td style={{width: 554}}></td>
                                            </tr>
                                            <tr style={{height: 1}}>
                                                <td
                                                    style={{
                                                        pointerEvents: "auto",
                                                        backgroundColor: "#FFFFFF",
                                                        borderTop: "1px solid #000000"
                                                    }}
                                                />
                                            </tr>
                                            <tr style={{height: 19}}>
                                                <td/>
                                            </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                            </td>
                            <td/>
                        </tr>
                        <tr style={{height: 10}}>
                            <td colSpan={20}/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td
                                colSpan={9}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: "11.5px",
                          lineHeight: "1.1640625"
                      }}
                  >
                    Mediante prova de pagamento da coima
                  </span>
                            </td>
                            <td
                                colSpan={3}
                                style={{
                                    textIndent: 5,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: "11.5px",
                          lineHeight: "normal"
                      }}
                  >
                    Text Field
                  </span>
                            </td>
                            <td
                                colSpan={6}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: "11.5px",
                          lineHeight: "1.1640625"
                      }}
                  >
                    ,relativa ao auto de contra-ordenação
                  </span>
                            </td>
                            <td/>
                        </tr>
                        <tr style={{height: 30}}>
                            <td/>
                            <td colSpan={18}>
                                <div
                                    style={{
                                        width: "100%",
                                        height: "100%",
                                        position: "relative"
                                    }}
                                >
                                    <div
                                        style={{
                                            position: "absolute",
                                            overflow: "hidden",
                                            width: "100%",
                                            height: "100%"
                                        }}
                                    >
                                        <table
                                            cellPadding={0}
                                            cellSpacing={0}

                                            style={{
                                                emptyCells: "show",
                                                width: "100%",
                                                borderCollapse: "collapse"
                                            }}
                                        >
                                            <tbody>
                                            <tr role="none" style={{height: 0}}>
                                                <td style={{width: 554}}></td>
                                            </tr>
                                            <tr style={{height: 30}}>
                                                <td
                                                    style={{
                                                        pointerEvents: "auto",
                                                        textIndent: 80,
                                                        textAlign: "justify"
                                                    }}
                                                >
                              <span
                                  style={{
                                      fontFamily: "Arial Rounded MT Bold",
                                      color: "#000000",
                                      fontSize: "11.5px",
                                      lineHeight: "normal"
                                  }}
                              >
                                , o arguido poderá levantar o(s) documento(s)
                                provisoriamente apreendido(s), nos seguintes
                                períodos, a contar da data de apreensão:
                              </span>
                                                </td>
                                            </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                    <div
                                        style={{
                                            position: "relative",
                                            width: "100%",
                                            height: "100%",
                                            pointerEvents: "none"
                                        }}
                                    >
                                        <table
                                            cellPadding={0}
                                            cellSpacing={0}

                                            style={{
                                                emptyCells: "show",
                                                width: "100%",
                                                borderCollapse: "collapse"
                                            }}
                                        >
                                            <tbody>
                                            <tr role="none" style={{height: 0}}>
                                                <td style={{width: 88}}></td>
                                                <td style={{width: 466}}></td>
                                            </tr>
                                            <tr style={{height: 20}}>
                                                <td
                                                    style={{
                                                        pointerEvents: "auto",
                                                        textIndent: 0,
                                                        textAlign: "left"
                                                    }}
                                                >
                              <span
                                  style={{
                                      fontFamily: "Arial Rounded MT Bold",
                                      color: "#000000",
                                      fontSize: "11.5px",
                                      lineHeight: "1.1640625"
                                  }}
                              >
                                Text Field
                              </span>
                                                </td>
                                                <td/>
                                            </tr>
                                            <tr style={{height: 10}}>
                                                <td colSpan={2}/>
                                            </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                            </td>
                            <td/>
                        </tr>
                        <tr style={{height: 30}}>
                            <td/>
                            <td
                                colSpan={18}
                                style={{textIndent: 10, textAlign: "justify"}}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: "11.5px",
                          lineHeight: "normal"
                      }}
                  >
                    - o Titulo de Condução poderá ser levantado nos primeiros
                    cinco dias úteis no DIVISÃO DE TRÂNSITO E SEGURANÇA
                    RODOVIÁRIA;
                  </span>
                            </td>
                            <td/>
                        </tr>
                        <tr style={{height: 22}}>
                            <td colSpan={20}/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td
                                colSpan={4}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: "11.5px",
                          lineHeight: "1.1640625"
                      }}
                  >
                    O Autuante:
                  </span>
                            </td>
                            <td
                                colSpan={12}
                                style={{
                                    borderBottom: "1px solid #000000",
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            />
                            <td colSpan={3}/>
                        </tr>
                        <tr style={{height: 8}}>
                            <td colSpan={20}/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td
                                colSpan={4}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: 12,
                          lineHeight: "1.1640625"
                      }}
                  >
                    O Arguido/Condutor:
                  </span>
                            </td>
                            <td
                                colSpan={12}
                                style={{
                                    borderBottom: "1px solid #000000",
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            />
                            <td colSpan={3}/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td
                                colSpan={17}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: 12,
                          lineHeight: "1.1640625"
                      }}
                  >
                    Certifica-se que o Arguido/Condutor se recusou a
                    receber/assinar a notificação
                  </span>
                            </td>
                            <td colSpan={2}/>
                        </tr>
                        <tr style={{height: 20}}>
                            <td/>
                            <td
                                colSpan={4}
                                style={{
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            >
                  <span
                      style={{
                          fontFamily: "Arial Rounded MT Bold",
                          color: "#000000",
                          fontSize: 12,
                          lineHeight: "1.1640625"
                      }}
                  >
                    A(s) Testemunha(s):
                  </span>
                            </td>
                            <td
                                colSpan={12}
                                style={{
                                    borderBottom: "1px solid #000000",
                                    textIndent: 0,
                                    verticalAlign: "middle",
                                    textAlign: "left"
                                }}
                            />
                            <td colSpan={3}/>
                        </tr>
                        <tr style={{height: 32}}>
                            <td colSpan={20}/>
                        </tr>
                        </tbody>
                    </table>
                </td>
                <td width="50%"/>
            </tr>
            </tbody>
        </table>
    )

}
